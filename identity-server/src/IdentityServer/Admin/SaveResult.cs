...
public static class SaveResult
{
    ...
    public static SaveResult<TId> Failure<TId>(params AdminError[] errors) where TId : notnull =>
        ...
        {
            IsSuccess = false,
            Errors = new AdminErrorCollection(errors ?? [])
        };
}
...
public record SaveResult<TId> where TId : notnull
{
    ...
    /// </summary>
    public AdminErrorCollection? Errors { get; internal set; }
...
#pragma warning disable CA2225
    ...
    {
        IsSuccess = false,
        Errors = new AdminErrorCollection([error])
    };

    /// <inheritdoc/>
    public override string ToString() => IsSuccess
        ? $"Saved {typeof(TId).Name} with id {Id}, Version {Version}"
        : $"Failed to save {typeof(TId).Name}. Errors: {Errors}";
