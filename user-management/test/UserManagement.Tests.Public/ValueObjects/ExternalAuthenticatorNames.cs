...
public static class ExternalAuthenticatorNames
{
    ...
    {
        var ex = Record.Exception(() => _ = ExternalAuthenticatorName.Create(input));
    ...
    {
        const string input = $"{nameof(ExternalAuthenticatorName)}1";
        var instance = ExternalAuthenticatorName.Create(input);
