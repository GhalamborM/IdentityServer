...
{
    public static TheoryData<string> InvalidInputs { get; } = ["", " ", "ab", "abc", new string('x', 321)];

    [Theory]
    [MemberData(nameof(InvalidInputs))]
    public static void CannotParseInvalidInputs(string input)
    {
        var ex = Record.Exception(() => _ = EmailAddress.Create(input));
    ...
    public static void String_is_input()
    {
        ...
        var instance = EmailAddress.Create(input);
